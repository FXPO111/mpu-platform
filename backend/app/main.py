...
async def correlation_middleware(request: Request, call_next):
    ...
    logger.info("request", correlation_id=correlation_id, path=request.url.path, method=request.method, duration_ms=(time.perf_counter() - start) * 1000)
    return response
...
async def rl_handler(request: Request, exc: RateLimitExceeded):
    return JSONResponse(status_code=429, content={"error": {"code": "RATE_LIMIT", "message": "Too many requests", "details": {}}})
